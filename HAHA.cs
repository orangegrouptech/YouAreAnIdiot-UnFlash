...
namespace YouAreAnIdiot_UnFlash
{
    public partial class HAHA : Form
    {
        ...
        private void Timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                this.MoveX.Text = Conversions.ToString(checked(-(1 + (int)Math.Round((double)unchecked(34f * VBMath.Rnd()))) * 1));
                if (Conversions.ToDouble(this.MoveY.Text) < 0.0)
                    this.MoveY.Text = Conversions.ToString(checked(-(1 + (int)Math.Round((double)unchecked(34f * VBMath.Rnd()))) * 1));
